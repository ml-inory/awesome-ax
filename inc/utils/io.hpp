...
#include "utils/ax_algo_def.h"
#include "ax_sys_api.h"
#include "ax_engine_type.h"

namespace axALGO {
typedef enum {
    ALGO_IO_BUFFER_STRATEGY_DEFAULT,
    ALGO_IO_BUFFER_STRATEGY_CACHED
} ALGO_IO_BUFFER_STRATEGY_T;

#define ALGO_IO_CMM_ALIGN_SIZE 128
...
static inline AX_S32 query_model_input_size(const AX_ENGINE_IO_INFO_T* io_info, std::array<int, 2> &input_size,
    ...
    ALOGI("eLayout:%d, eDataType:%d, channel:%d, height:%d, width:%d, size:%d, data_type_size:%d",
...
static inline AX_S32 alloc_engine_buffer(const std::string& token, const std::string& appendix, size_t index, const AX_ENGINE_IOMETA_T* pMeta, AX_ENGINE_IO_BUFFER_T* pBuf, ALGO_IO_BUFFER_STRATEGY_T eStrategy = ALGO_IO_BUFFER_STRATEGY_DEFAULT) {
    AX_S32 ret = -1;
    if (eStrategy != ALGO_IO_BUFFER_STRATEGY_DEFAULT && eStrategy != ALGO_IO_BUFFER_STRATEGY_CACHED) {
    ...
    if (eStrategy == ALGO_IO_BUFFER_STRATEGY_CACHED) {
        ret = AX_SYS_MemAllocCached((AX_U64*)&pBuf->phyAddr, &pBuf->pVirAddr, pBuf->nSize, ALGO_IO_CMM_ALIGN_SIZE, (const AX_S8*)token_name.c_str());
    }
    else {
        ret = AX_SYS_MemAlloc((AX_U64*)&pBuf->phyAddr, &pBuf->pVirAddr, pBuf->nSize, ALGO_IO_CMM_ALIGN_SIZE, (const AX_S8*)token_name.c_str());
    }
...
static inline int prepare_io(const std::string& token, const AX_ENGINE_IO_INFO_T* info, AX_ENGINE_IO_T &io, ALGO_IO_BUFFER_STRATEGY_T strategy) {
    auto ret = 0;
...
static inline int prepare_io(const std::string& token,
                                ...
                                const ALGO_IO_BUFFER_STRATEGY_T &strategy) {
    AX_S32 ret = 0;
    memset(&io, 0, sizeof(io));
